"""
OS-backed local secret storage for integration API keys.

Keys never leave the machine through this module. Used by the Blender addon
preferences path when the MCP server or helper scripts need a safe store;
the addon itself prefers Blender's PASSWORD fields + this file-backed vault
for persistence outside .blend files.

Windows: DPAPI (CryptProtectData) via ctypes
macOS:   prioritizes keyring if installed, else Fernet file with machine salt
Linux:   keyring if installed, else Fernet file with XDG config permissions
"""
